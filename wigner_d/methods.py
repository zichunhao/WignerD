...
def wigner_D_from_d(
    ...
    for row, m_prime in enumerate(m_range):
        for col, m in enumerate(m_range):
            mat[row, col] = np.exp(-1j * (m_prime *
                                   alpha + m * gamma)) * wigner_d[row, col]
...
def wigner_d_from_D(
    ...
    for row, m_prime in enumerate(m_range):
        for col, m in enumerate(m_range):
            mat[row, col] = np.exp(
                1j * (m_prime * alpha + m * gamma)) * wigner_D[row, col]
...
def wigner_D_dict_from_d_dict(
    wigner_d_dict: Dict[float, np.ndarray], 
    alpha: float, 
    gamma: float
) -> Dict[float, np.ndarray]:
